...
public sealed class DatabaseSafetyTests : IDisposable {
	...
	string ReadOnlyEntryName(string path) {
		...
		Assert.Equal("VDFDB002"u8.ToArray(), header.ToArray());
		Span<byte> integer = stackalloc byte[sizeof(int)];
		stream.ReadExactly(integer); // database version
		stream.ReadExactly(integer); // image hash pipeline version
		stream.ReadExactly(integer);
		int entryLength = BinaryPrimitives.ReadInt32LittleEndian(integer);
		Assert.True(entryLength >= 0);
		byte[] payload = new byte[entryLength];
		stream.ReadExactly(payload);
		FileEntry entry = MemoryPackSerializer.Deserialize<FileEntry>(payload)!;
