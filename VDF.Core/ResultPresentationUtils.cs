...
namespace VDF.Core {
	...
	internal static class ResultPresentationUtils {
		...
		internal static bool PathMatchesFilter(string path, string needle) {
			...
			// MatchesSimpleExpression treats backslashes as escapes, so raw Windows
			// paths in wildcard filters must escape each separator first (#864).
			string pattern = needle.Replace("\\", "\\\\");
