...
> **Note:** Partial clip detection requires audio tracks in both files. Videos without audio are skipped. Fast Balanced still runs the existing exact sliding comparison and visual confirmation on retained candidates, so it does not add false positives; however, approximate candidate search can miss a true match. Use Exact for zero intentional candidate loss.
